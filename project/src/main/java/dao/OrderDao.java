...
public class OrderDao extends BaseDao {
    ...
    public List<Product> getTopProducts(int limit) {
        String sql = "select p.product_id as productId, p.product_name as productName, p.product_price as productPrice, p.image_url as imageUrl, sum (oi.quantity) as sold, sum(oi.quantity*oi.price) as revenue from order_items oi join products p on oi.product_id = p.product_id group by p.product_id order by sold desc limit :limit";
        return getJdbi().withHandle(handle ->
                handle.createQuery(sql)
                        .bind("limit", limit)
